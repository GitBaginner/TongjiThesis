...
\subsection{评估方案}
对通用语言控制模型的性能评估采用在线仿真闭环测试方式，每个模型在每个任务上独立运行500个回合。为全面评估轻量LLM作为通用控制器的多任务控制能力，本节以多任务混合微调为主要实验，将单任务独立微调作为消融对照实验。评估指标包括：
...
如\cref{tab:experiment-results}及\cref{fig:success-rate-comparison}所示，在多任务混合微调设定下，Qwen3.5-0.8B以54.9\%的平均成功率取得了最优性能，在PickAndPlace任务上达到65.4\%，为所有模型中最高。Gemma3-1B以50.2\%的平均成功率位居第二，在PickAndPlace任务上达到56.6\%，展现出显著的多任务学习能力。Llama3.2-1B与Qwen3-0.6B紧随其后，平均成功率分别为41.95\%与40.75\%。OLMo2-1B的平均成功率为17.75\%，较其单任务基线有显著提升。Phi-1.5的平均成功率仅为3.9\%，表明该基座模型的预训练表征与数值密集型动作生成任务存在较大差距。作为性能上界参考，MLP专家模型的平均成功率达到96.9\%，所有LLM采取的控制策略与之相比仍存在明显差距。
...
通过对比同一模型在多任务与单任务两种微调设定下的性能差异，可以观察到多任务训练对不同基座模型产生了差异化的影响。其中，Gemma3-1B表现出最为突出的多任务增益，平均成功率从单任务设定的33.7\%跃升至多任务设定的50.2\%，提升了16.5个百分点。该提升主要体现在PickAndPlace与Slide两个复杂任务上，分别从20.6\%和5.4\%提升至56.6\%和26.4\%，表明跨任务知识迁移对中等表现模型具有显著的正向效应。
...
值得关注的是，在单任务设定中表现最优的Qwen3.5-0.8B在多任务微调中也获得了5.0个百分点的提升，平均成功率从49.9\%提高至54.9\%。该提升主要来自PickAndPlace任务上23.4个百分点的增长（42.0\%$\to$65.4\%），而Push任务则出现了6.0个百分点的性能衰减。这表明即使是高表现模型，也能从多任务训练中获益，尽管部分任务间仍存在参数竞争。

Qwen3-0.6B在多任务微调中出现了4.0个百分点的平均成功率下降，主要体现在PickAndPlace任务从44.4\%降至30.4\%。这种现象揭示了在模型容量有限时，多任务训练中的资源分配竞争可能抑制部分任务的表现。

整体而言，多任务混合微调在大多数模型上表现出正向效果，其中Gemma3-1B与Llama3.2-1B获得了最大的性能增益，验证了单一LLM同时掌握多项操作技能的可行性。但Qwen3-0.6B的性能下降表明，简单的数据混合策略在模型容量有限时仍存在瓶颈，后续可通过任务条件化的微调策略或模型扩展加以改善。
...
由\cref{tab:latency-vllm}可知，vLLM推理引擎对所有模型均实现了不同程度的推理加速。其中，Qwen3-0.6B的加速效果最为显著，推理延迟从320.80ms降至189.1ms，加速比达到1.70倍；Qwen3.5-0.8B的延迟从466.60ms降至338.8ms，加速比为1.38倍。Gemma3-1B的加速效果相对有限，加速比仅为1.16倍，这可能与其模型内部的注意力计算模式对PagedAttention的适配程度有关。
...
\subsection{小结}
本章实验表明，在选定的6个轻量级基座模型中，基于Qwen系列模型的LoRA微调方案在机械臂多任务操作控制上取得了最优的控制性能。在多任务混合微调设定下，Qwen3.5-0.8B以54.9\%的平均成功率领先，Gemma3-1B与Llama3.2-1B分别获得16.5与12.85个百分点的多任务增益，验证了单一LLM同时掌握多项操作技能的可行性与跨任务知识迁移的正向效应。作为消融对照，单任务微调中Qwen3-0.6B以44.75\%的平均成功率取得最优性能，但其在多任务微调中出现了4.0个百分点的性能下降，表明简单的数据混合策略在模型容量有限时存在瓶颈。所有模型在两种设定下的推理延迟均处于1\textasciitilde10 Hz的可接受范围内，验证了vLLM部署方案的实用性。然而，所有LLM控制策略与MLP专家模型之间仍存在较大性能差距，后续可通过任务条件化的微调策略或扩大模型规模加以改善。